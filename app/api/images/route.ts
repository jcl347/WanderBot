...
const DestSchema = z.object({
  ...
  // ✨ NEW: short, city-free terms coming from the LLM (e.g., ["South Beach","Wynwood","nightlife"])
  image_terms: z.array(z.string()).optional(),
...
// --- NEW: sanitize LLM "image_terms"
function simplifyToTerms(v: unknown, fallbackFromPOIs: string[]): string[] {
  ...
  // keep 1–2 word phrases, strip city names / commas / long phrases
  const cleaned = raw
    .map((t) =>
      t
        .replace(/[|,]/g, " ")
  ...
  // fallback: take POI names (also reduced to 1–2 words)
  let terms = cleaned;
  if (terms.length < 6) {
    const poiTerms = fallbackFromPOIs
      .map((n) =>
        n
          .replace(/[|,]/g, " ")
          .replace(/\s+/g, " ")
      ...
      .filter(Boolean);
    const set = new Set([...terms, ...poiTerms]);
...
EXAMPLE DESTINATION (shape only):
{
  ...
  "image_terms": ["street art","skate park","food trucks","hike","concert","museum","nightlife","sunset"]
}
`.trim();
...
Airfare guidance:
- Round-trip economy, USD, return **monthBreakdown** covering every month in ${monthWindow}.
- Reasonable ranges: short-haul $120–$350; medium $180–$650; long $400–$1200; very long $900–$2000+. Seasonality & festivals affect peaks.
- The overall "avgUSD" per traveler is the **mean of the monthly values**.

Hard requirements per destination:
- "narrative": explain WHY it fits the group; include a **2–3 bullet micro-itinerary in prose**.
- "per_traveler_fares": each traveler must include { "travelerName", "from" (city), "avgUSD", "monthBreakdown":[…all months…] }.
- "months": cover every month; each "note" names a specific event/festival when applicable.
- Map: include "map_center" and **6–10 "map_markers"** with numeric [lat,lon] and short "blurb".
- **"image_terms": array of 8–14 SHORT terms (1–2 words each, no city names)** derived from the micro-itinerary and the group's interests (e.g., "South Beach","Wynwood","nightlife","art deco","cafe","food market","museum","hike"). These terms will be combined with the city name on our side to fetch photos.

If anyone "dislikes travel", bias near ${anchor}. Otherwise balance cost vs. interests.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!json || typeof json !== "object") throw new Error("Model returned non-object JSON");
    ...
    json.destinations = json.destinations.map((d: any, i: number) => {
      ...
      enforceFareCoverageAndSmooth(fares, body.timeframe.startMonth, body.timeframe.endMonth);

      const monthsRaw = normalizeMonths(base.months, body.timeframe.startMonth);
      const months = expandMonthsToRange(monthsRaw, body.timeframe.startMonth, body.timeframe.endMonth);
      ...
      // ✨ image terms sanitation / fallback from POIs
      const markerNames = (cleanedMarkers || []).map((m) => m.name);
      const image_terms = simplifyToTerms(base.image_terms, markerNames);
      ...
      return {
        ...
        image_terms, // <- used by the UI for LiveCollage
        ...
        analysis: base, // keep full raw for debugging if needed
      };
    });
    ...
    console.log(
      `[plan ${reqId}] image_terms counts=`,
      parsed.destinations.map((d) => ({ slug: d.slug, n: d.image_terms?.length || 0 }))
    );
